...
# Ordinary exit cleanup must not disturb it: still exactly one PgSleep
# record in the now-orphaned ring, neither lost nor duplicated.
$A->quit;
$node->poll_query_until(
	'postgres',
...
is( $node->safe_psql(
	...
	'1',
	"A's orphaned trace ring still holds exactly the one PgSleep wait "
	  . "after A's exit");
