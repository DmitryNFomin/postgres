...
pwet_wait_begin_impl(uint32 wait_event_info, bool chain)
{
	 ...
	 * already-allocated ring, no allocation/lock/wait/ereport.
	 */
...
pwet_wait_end_impl(uint32 wait_event_info, bool chain)
{
	 ...
	 * recorded.
	 */
	PwetStats  *state = pwet_rec_stats;
	...
	if (state != NULL)
	{
		...
		if (event != 0 && !INSTR_TIME_IS_ZERO(pwet_wait_start))
		{
			 ...
			 * lock-free, exactly like the stats accounting above.
			 * pwet_rec_trace is non-NULL exactly when the old three-way
			 * test (pwet_capture == TRACE, !pwet_trace_writes_disabled,
			 * pwet_my_trace != NULL) held -- see
			 * pwet_update_rec_pointers()'s comment.
			 */
			if (pwet_rec_trace != NULL)
			{
				PwetTraceState *trace = pwet_rec_trace;
				uint64		pos;
				PwetTraceRecord *rec;
				uint32		seq;
