...
KIT_DIR=${1:?usage: run-worker.sh R3_KIT_DIR OUTPUT_DIR SOCKET_DIR [full|smoke]}
OUT_DIR=${2:?usage: run-worker.sh R3_KIT_DIR OUTPUT_DIR SOCKET_DIR [full|smoke]}
SOCKET_DIR=${3:?usage: run-worker.sh R3_KIT_DIR OUTPUT_DIR SOCKET_DIR [full|smoke]}
RUN_MODE=${4:-full}
...
PREFIX_V9="$KIT_DIR/work/install/v9-ref"
PREFIX_V10="$KIT_DIR/work/install/v10opt"
...
run_session() {
  local session_index=$1 pair_index=$2 pair_position=$3
  local build=$4 sequence=$5 pgbench_seed=$6
  CURRENT_SESSION="index=$session_index pair=$pair_index build=$build sequence=$sequence"
  local prefix=$PREFIX_V9
  [[ "$build" == v9 ]] || prefix=$PREFIX_V10
  ...
  cp -a "$DATA_ROOT/pair-fixture-$pair_index" "$datadir" ||
    die "could not clone the neutral pair fixture"
...
PY
    ...
    rm -rf -- "$datadir"
    CURRENT_SESSION=""
    log "completed smoke session build=$build sequence=$sequence"
    return
  fi

  local metadata
  metadata=$(python_run - "$session_index" "$pair_index" "$pair_position" \
    "$build" "$sequence" "$pgbench_seed" "$POSTMASTER_PID" \
    "$measured_pgbench_pid" "$SERVER_CPUS" "$PGBENCH_CPUS" \
    "logs/aggregate-$session_index" "logs/server-$session_index.log" <<'PY'
import json
import sys
keys = (
    "session_index", "pair_index", "pair_position", "build", "sequence",
...
prepare_pair_fixture() {
  local pair_index=$1
  local fixture="$DATA_ROOT/pair-fixture-$pair_index"
  local setup_log="$LOG_DIR/server-setup-pair-$pair_index.log"
  local init_log="$LOG_DIR/init-pair-$pair_index.log"
  [[ ! -e "$fixture" ]] || die "pair fixture already exists"
  init_cluster "$fixture" "$init_log"
  start_server "$PREFIX_A" "$fixture" "$setup_log" no
  PGHOST="$SOCKET_DIR" PGPORT="$PORT" PGUSER="$EXECUTOR_USER" \
    run_from_prefix "$PREFIX_A" taskset -c "$PGBENCH_CPUS" \
    "$PREFIX_A/bin/pgbench" -i -q -s 100 postgres \
    >"$LOG_DIR/pgbench-init-pair-$pair_index.log" 2>&1 ||
    die "pgbench initialization failed"
  psql_for "$PREFIX_A" -qAtc "CHECKPOINT" >/dev/null ||
    die "pair-fixture checkpoint failed"
  stop_server_checked "neutral pair-fixture setup"
}
...
[[ "$(sha256sum "$KIT_DIR/PACKAGE-MANIFEST.sha256" | awk '{print $1}')" \
  ...
  die "r3 package manifest differs"
[[ "$(sha256sum "$KIT_DIR/work/manifest.json" | awk '{print $1}')" \
   == "$EXPECTED_BUILD_MANIFEST_SHA256" ]] ||
  die "r3 build manifest differs"
for prefix in "$PREFIX_A" "$PREFIX_V9" "$PREFIX_V10"; do
  [[ -x "$prefix/bin/postgres" && -x "$prefix/bin/pgbench" ]] ||
    die "verified r3 installation is missing: $prefix"
done
python_run "$SCRIPT_DIR/verify_installs.py" \
  "$KIT_DIR/work/manifest.json" "$PREFIX_A" "$PREFIX_V9" "$PREFIX_V10" ||
  die "reused r3 installation trees differ from the build manifest"
...
[[ "$SERVER_CPUS" == 1-63:2 && "$PGBENCH_CPUS" == 0-14:2 ]] ||
  die "canonical CPU masks differ"
python_run "$AFFINITY_HELPER" \
...
if [[ "$RUN_MODE" == smoke ]]; then
  prepare_pair_fixture 1
  run_session 1 1 1 v9 A 1
  run_session 2 1 2 v10 B 1
  [[ "$(wc -l <"$EVENTS" | tr -d ' ')" == 48 ]] ||
    die "smoke event count differs"
  [[ "$(wc -l <"$PROOFS" | tr -d ' ')" == 17 ]] ||
    die "smoke proof count differs"
  rm -rf -- "$DATA_ROOT/pair-fixture-1"
  trap - EXIT ERR
  on_exit
...
for line in "${cells[@]}"; do
  ...
  IFS=, read -r \
    session_index pair_index pair_position build sequence pgbench_seed \
    <<<"$line"
  [[ "$session_index" == "$expected_session" &&
     "$pair_index" =~ ^([1-9]|1[0-6])$ &&
     "$pair_position" =~ ^[12]$ &&
     "$build" =~ ^v(9|10)$ &&
     "$sequence" =~ ^[AB]$ &&
  ...
  if [[ "$pair_position" == 1 ]]; then
    prepare_pair_fixture "$pair_index"
  fi
  run_session \
    "$session_index" "$pair_index" "$pair_position" "$build" "$sequence" \
    "$pgbench_seed"
  if [[ "$pair_position" == 2 ]]; then
    rm -rf -- "$DATA_ROOT/pair-fixture-$pair_index"
  fi
