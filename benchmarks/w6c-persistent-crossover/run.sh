...
KIT_DIR=${1:?usage: ./run.sh /path/to/wet-v10-baremetal-r3}
[[ $# -eq 1 ]] || {
  echo "usage: ./run.sh [--smoke] /path/to/wet-v10-baremetal-r3" >&2
  exit 1
}
...
[[ "$(sha256sum "$KIT_DIR/PACKAGE-MANIFEST.sha256" | awk '{print $1}')" \
  ...
  die "this is not the bound r3 package"
(
  cd "$KIT_DIR"
...
[[ "$(stat -c '%U' "$KIT_DIR/work/manifest.json")" == "$(id -un)" ]] ||
  die "r3 builds belong to a different executor"
...
R3_MANIFEST_SHA256=$(sha256sum \
  "$KIT_DIR/PACKAGE-MANIFEST.sha256" | awk '{print $1}')
SMOKE_MARKER="$SOURCE_DIR/.SMOKE-PASS"
SMOKE_ID="schema=1 crossover=$CROSSOVER_MANIFEST_SHA256 r3=$R3_MANIFEST_SHA256 executor=$(id -un) hostname=$(hostname)"
...
log "Staging: $STAGE"
if [[ "$RUN_MODE" == full ]]; then
  log "Waiting for the verified r3 idle gate."
...
if [[ "$RUN_MODE" == full ]]; then
  log "Beginning 32 persistent-backend sessions in 16 adjacent v9/v10 pairs."
else
