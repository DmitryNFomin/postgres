#!/usr/bin/env python3
"""Validate and analyze the persistent-backend W6c crossover."""
...
def validate_schedule(rows: list[dict[str, str]]) -> None:
    ...
    require(
        [int(row["session_index"]) for row in rows]
        == list(range(1, SESSIONS + 1)),
    ...
    for pair_index in range(1, PAIRS + 1):
        ...
        require(
            len(pair) == 2
            and [int(row["session_index"]) for row in pair]
            == [pair_index * 2 - 1, pair_index * 2]
            and [int(row["pair_position"]) for row in pair] == [1, 2]
            and {row["build"] for row in pair} == set(BUILDS)
            and len({row["sequence"] for row in pair}) == 1
            and len({row["pgbench_seed"] for row in pair}) == 1,
            "schedule pair {} is malformed".format(pair_index),
        )
        require(
            int(pair[0]["pgbench_seed"]) == pair_index,
            "schedule seed does not equal pair index",
        )
        combinations[(pair[0]["sequence"], pair[0]["build"])] += 1
    require(
        combinations
        == Counter({
            ("A", "v9"): PAIRS // 4,
        ...
        "sequence and first-build allocation is not balanced",
    )
...
def validate_results(
    ...
    require(
        len(rows) == SESSIONS * BLOCKS_PER_SESSION,
        "results are incomplete",
    )
    schedule_by_session = {
        int(row["session_index"]): row for row in schedule
    }
    ...
    for row in rows:
        ...
        require(
            ...
            and 0
            <= float(row["pgbench_capacity_fraction"])
            < MAX_PGBENCH_CAPACITY_FRACTION
            and row["server_cpus"] == SERVER_CPUS
    ...
    for session_index, session_rows in by_session.items():
        ...
        require(
            ...
            and [(row["mode"], row["role"]) for row in session_rows]
            == list(expected)
...
def validate_protocol(root: Path) -> None:
    ...
    seed_text = (root / "worker/seed.txt").read_text(
        encoding="ascii"
    ).strip()
    require(
        seed_text.isdecimal(),
        "schedule seed is malformed",
    )
    ...
    require(
        protocol == expected,
        "protocol or bound provenance differs",
    )
...
def validate_host_state(root: Path) -> None:
    ...
    require(
        before_path.is_file()
        and not before_path.is_symlink()
        and after_path.is_file()
        and not after_path.is_symlink(),
    ...
    for field in (
        ...
        require(
            ...
            "host state changed: {}".format(field),
        )
...
def validate_events(root: Path, rows: list[dict[str, str]]) -> None:
    ...
    require(
        len(events) == SESSIONS * BLOCKS_PER_SESSION * 3,
        "event count differs",
    )
    expected_rows = {
        (int(row["session_index"]), int(row["block_index"])): row
        for row in rows
    }
    ...
    for event in events:
        ...
        require(
            type(timestamp) is int and timestamp > previous_timestamp,
            "event timestamps are not strictly increasing",
        )
        previous_timestamp = timestamp
        key = (event.get("session_index"), event.get("block_index"))
        require(
            key in expected_rows
            and event.get("mode") == expected_rows[key]["mode"],
    ...
    for key, block_events in by_block.items():
        ...
        require(
            ...
            and block_events[1]["timestamp_ns"]
            == int(row["measurement_start_ns"])
            and block_events[2]["timestamp_ns"]
            == int(row["measurement_end_ns"]),
...
def validate_proofs(root: Path, rows: list[dict[str, str]]) -> None:
    ...
    require(
        len(proofs) == SESSIONS * BLOCKS_PER_SESSION,
        "mode proofs are incomplete",
    )
    expected = {
        (int(row["session_index"]), int(row["block_index"])): row
        for row in rows
    ...
    require(
        len(expected) == SESSIONS * BLOCKS_PER_SESSION,
    ...
    for proof in proofs:
        ...
        if proof["mode"] == "off":
            require(
                (clients, before, after, trace) == (0, 0, 0, 0),
                "off-mode proof shows recording",
            )
        elif proof["mode"] == "stats":
            require(
                clients == CLIENTS and 0 < before < after and trace == 0,
...
def validate_evidence_paths(root: Path, rows: list[dict[str, str]]) -> None:
    for session_index in range(1, SESSIONS + 1):
        session = [
            row for row in rows
            if int(row["session_index"]) == session_index
        ]
        digest = session[0]["backend_pid_sha256"]
        backend_path = (
            root / "worker/backend-pids"
            / "session-{}.txt".format(session_index)
        )
        require(
            backend_path.is_file()
            and not backend_path.is_symlink()
            and sha256(backend_path) == digest,
            "backend PID evidence differs",
        )
        server_path = root / "worker" / session[0]["server_log"]
        require(
            server_path.is_file() and not server_path.is_symlink(),
        ...
        require(
            all(
                all(str(actual[field]) == expected[field]
...
def session_estimates(rows: list[dict[str, str]]) -> list[dict]:
    ...
    for session_index in range(1, SESSIONS + 1):
        ...
        stats_off = [
            by_role["off_stats_before"],
            by_role["off_stats_after"],
        ]
        trace_active = [by_role["trace_first"], by_role["trace_second"]]
        trace_off = [
            by_role["off_trace_before"],
            by_role["off_trace_after"],
        ]
        ...
        item = {
            ...
            "pair_index": int(session[0]["pair_index"]),
            "pair_position": int(session[0]["pair_position"]),
            "build": session[0]["build"],
            ...
            "placebo_log": (
                math.log(by_role[placebo_roles[1]])
                - math.log(by_role[placebo_roles[0]])
            ),
            "stats_bracket_drift_log": (
                math.log(by_role["off_stats_after"])
                - math.log(by_role["off_stats_before"])
            ),
            "trace_bracket_drift_log": (
                math.log(by_role["off_trace_after"])
                - math.log(by_role["off_trace_before"])
            ),
...
def analyze(root: Path) -> dict:
    ...
    by_build = {}
    for build in BUILDS:
        selected = [item for item in estimates if item["build"] == build]
        by_build[build] = {
            "stats_overhead": interval(
                [item["stats_overhead_log"] for item in selected],
                EQUIVALENCE_MARGIN_PERCENT,
            ),
            "trace_overhead": interval(
                [item["trace_overhead_log"] for item in selected],
                EQUIVALENCE_MARGIN_PERCENT,
            ),
            "trace_vs_stats": interval(
                [item["trace_vs_stats_log"] for item in selected],
                EQUIVALENCE_MARGIN_PERCENT,
            ),
            "off_off_placebo": interval(
                [item["placebo_log"] for item in selected],
                PLACEBO_MARGIN_PERCENT,
            ),
            "stats_bracket_drift": interval(
                [item["stats_bracket_drift_log"] for item in selected],
                PLACEBO_MARGIN_PERCENT,
            ),
            "trace_bracket_drift": interval(
                [item["trace_bracket_drift_log"] for item in selected],
                PLACEBO_MARGIN_PERCENT,
            ),
        }
    ...
    for item in estimates:
        by_pair[item["pair_index"]][item["build"]] = item
    require(
        set(by_pair) == set(range(1, PAIRS + 1))
        and all(set(pair) == set(BUILDS) for pair in by_pair.values()),
        "paired session estimates are incomplete",
    )
    paired_fields = {
        "stats_overhead_change": "stats_overhead_log",
        "trace_overhead_change": "trace_overhead_log",
        "off_throughput_change": "off_level_log",
        "stats_throughput_change": "stats_level_log",
        "trace_throughput_change": "trace_level_log",
    }
    v10_vs_v9 = {}
    for name, field in paired_fields.items():
        differences = [
            by_pair[pair_index]["v10"][field]
            - by_pair[pair_index]["v9"][field]
            for pair_index in range(1, PAIRS + 1)
        ]
        v10_vs_v9[name] = interval(
            differences,
            EQUIVALENCE_MARGIN_PERCENT,
        )

    noise_controls_passed = all(
        item[key]["classification"] == "equivalent"
        for item in by_build.values()
        for key in (
            "off_off_placebo",
            "stats_bracket_drift",
            "trace_bracket_drift",
    ...
    report = {
        "schema_version": 1,
        "design": "persistent-backend bracketed crossover",
        "pairs": PAIRS,
        "sessions": SESSIONS,
        "blocks": len(rows),
        "suitability": {
    ...
    with (root / "session-estimates.csv").open(
        "x", newline="", encoding="utf-8"
    ) as stream:
        writer = csv.DictWriter(
            stream,
            fieldnames=tuple(estimates[0]),
            lineterminator="\n",
        )
        writer.writeheader()
    ...
    for build in BUILDS:
        for label, key in (
            ...
            item = by_build[build][key]
            lines.append(
                "| {} | {} | {:+.3f}% | [{:+.3f}%, {:+.3f}%] | {} |".format(
                    build,
                    label,
                    item["estimate_percent"],
                    item["lower_95_percent"],
                    item["upper_95_percent"],
                    item["classification"],
                )
            )
    lines.extend([
    ...
    for name, item in v10_vs_v9.items():
        lines.append(
            "| {} | {:+.3f}% | [{:+.3f}%, {:+.3f}%] | {} |".format(
                name.replace("_", " "),
                item["estimate_percent"],
    ...
    lines.extend([
        "",
        "The confidence intervals use sessions or adjacent v9/v10 session "
        "pairs as independent observations. One-second rows are never "
