...
- **Attach decisions use the becoming value; recording decisions use the
  current one (WP3 CI, run 34703751075).** `pwet_maybe_attach()`'s trace
  branch tested the stored `pwet_capture`, which guc.c has not yet updated
  when the assign hook runs, so `SET capture = trace` skipped the ring
  attach and then cleared `pwet_attach_needed`; every later attach call
  returned at its first line and the session never got a ring. All seven
  trace regress cases returned NULL on every platform, with a clean server
  log. The gates beside it (`pwet_can_attach`, `pwet_can_attach_trace`)
  were already correct, which is what made the inconsistency easy to miss.
  Rule: attach/claim decisions read `pwet_capture_effective`; hooks and
  marker writers read `pwet_capture`.
- **Do not poll for one specific background process (WP3 CI).** t/006
  failed only on the two Windows jobs, a different process each time
  (checkpointer on MinGW after 396 s, walwriter on MSVC), while the others
  recorded waits within 0.1 s and the region checks passed. An idle Windows
  server need not make a given auxiliary process wait inside the poll
  window. Tests must drive activity that forces the work, or assert over a
  set of equivalent process types, rather than waiting on one.
- **Three marker expectations were wrong, not the module (WP3 CI, run
  34706732942).** Verified against core source before changing anything:
  (a) for an explicit `COMMIT`, the markers are UtilityStart, UtilityEnd,
  TxnCommit — `EndTransactionBlock()` only sets TBLOCK_END inside the
  utility, and the commit (with its XACT_EVENT_COMMIT callbacks) happens in
  `finish_xact_command()` after ProcessUtility returns; there is no contrast
  with a plain utility statement. (b) `SELECT 1/0` raises at PLANNING time
  (`eval_const_expressions` → `evaluate_expr`), so no ExecStart marker
...
3. Demos: `blog/demo/run_demo_v2.sh` and `run_demo_trace.sh` re-run against the
   ...
   equivalent.
4. Re-measurement on the bare-metal host. **The owner runs it himself
   (2026-09-13); Claude hands over one self-contained kit when the assembled
   `wet-v8` is CI-green: a build script for the five configurations from a
   fixed commit, a run script, paste-able commands, the settings to check,
   and a request for one results archive back. Claude produces the tables.**
   (~3 h): configurations master, NULL (0001+0002), module-off, stats,
...
| WP3 | launched 2026-09-11 on `wet-v8-wp2b`. M1 ring/writer/readers a7e709759f5 and M2 orphan lifecycle e36829e9ac5 landed 2026-09-12; M3 (markers, attribution, regress) in progress. Review of M1/M2: seqlock port faithful to v6 (position-encoded identity, barriers both sides), orphan design free of V6-3's ordering trap, ring size forced to a power of two. Three items sent back and fixed in M3: (A) reserved-slot processes could never attach a ring, and never registered the exit callback; (B) rows emitted under the shared lock; (C) grants question — answered by checking v6, which restricts the own-session trace the same way and says why. Review of M3 confirmed: nesting depth correct (start writes then increments, end decrements then writes, so only the depth-0 pair closes a statement), marker state and depth reset at ring attach, and the end-of-statement marker now written after executor cleanup, fixing the sub-point of V6-6 the source review raised. CI run 34703751075: trace regress failed on every platform from one cause (attach tested the stored capture value; see §4.2b) and t/006 failed on Windows only from a test-robustness problem (polling one specific background process). Both sent back 2026-09-12 and fixed in 8412db0ac0a (attach now tests `pwet_capture_effective` and clears the retry flag only on success; the rule is documented once for every call site; Idle filtered out of the regress comparisons and deferred to a WP4b TAP case; t/006 polls a disjunction of background process types). CI run 34706732942: **t/006 green on every platform, 9–10 checks per job, and the ring records again**; three remaining trace-regress failures were wrong expectations (see §4.2b), fixed in 20bc081dbd2 (COMMIT ordering; no ExecStart for a planning-time error; PL/pgSQL function so the nested call is not inlined). **DONE 2026-09-13: CI run 34745232186 all green — both regress files pass on every platform and t/006 passes 8–10 checks per job.** Coverage dropped on purpose and moved to WP4b: the Idle marker (needs a deliberate client pause) and `pwet_marker_txn_abort()`'s depth reset (needs an error raised during execution, not planning) | WP2b | 4 |
...
| WP4b | TAP tests 005, 010, 011, 012 (trace level) — code done 2026-09-13 on `wet-v8-wp4b` (5ce93d72f25 adds an injection point in `pwet_wait_end()` between the write_pos advance and the seq stamp; 3d10f58e521 adds the four tests). Reviewed: the injection point is safe in a hook — the macro compiles to nothing without `USE_INJECTION_POINTS`, and with it an unattached point reads one atomic counter and returns without taking a lock (`InjectionPointCacheRefresh`); the depth-reset case correctly uses a PROCEDURE via `CALL`, since `CALL` goes through ProcessUtility and leaves exactly one nested level unclosed. CI run 34765154176: 005 (10), 011 (21), 006 (9–10) and both regress files green everywhere; 010 died on 5/6 platforms because the meson `'tap'` entry set no `enable_injection_points` env; 012's "no Idle between two statements on one line" failed on 3/6 because psql sends each `;`-terminated statement as its own message, so blocking is timing, not protocol. Both sent back 2026-09-13 and fixed (76080e7f5d1 wiring, e035e517730 case 2 rebuilt on `psql -c`, expectation re-derived: several statements in one message share an implicit transaction and commit once at the end, `use_implicit_block` in exec_simple_query, whereas psql-split statements commit individually — which is why the regress test expects two commits and this test one). **DONE 2026-09-13: CI run 34771018145 all green — 005 (10), 010 seqlock (5), 011 wrap (21), 012 markers (4), 006 (8–10) and both regress files on every platform** | WP3 | 2 |
| WP5 | docs page, xfunc paragraph, removal audit — **DONE 2026-09-13**: `wet-v8-wp5` 03dd07b9d65 (1516-line contrib page, registered in contrib.sgml/filelist.sgml) + 38fe0ae854d (xfunc paragraph and example on the timed pair) + my 82cc5367b57 (grants, after the ordering error in §4.2b). **CI run 34765707266 all green**, and the CompilerWarnings job's "Build documentation" step genuinely builds the SGML (that container has xmllint/xsltproc; the Meson jobs do not). Audit: only allowed paths touched, no v6 leftovers. Follow-up noted: 9 lines in `blog/demo/run_demo*.sh` still use v6 GUC names, to fix with the demo re-run | WP2/WP3 | 2 |
