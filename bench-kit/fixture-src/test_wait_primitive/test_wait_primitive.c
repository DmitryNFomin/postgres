...
PG_FUNCTION_INFO_V1(test_wait_primitive_lwlock_contention);
...
/*
 * Repeatedly contend on one existing core LWLock.  This is a qualification
 * fixture for the short-wait workload, not a microbenchmark: concurrent
 * callers continuously hand the lock between backends without adding an
 * artificial hold delay.
 */
Datum
test_wait_primitive_lwlock_contention(PG_FUNCTION_ARGS)
{
	int64		iterations = get_iterations(fcinfo);
