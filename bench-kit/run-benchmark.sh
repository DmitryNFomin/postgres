...
usage() {
  cat <<'EOF'
Usage:
  ./run-benchmark.sh          verify, preflight, build, smoke, run, collect
...
PHASE_START_EPOCH=$(date +%s)
write_status complete "raw archive ready; local analysis required: $archive"
log "RAW CAPTURE SUCCESS"
log "Raw archive: $archive"
log "Checksum: $archive.sha256"
log "Return both files unedited and analyze them on the local machine."
